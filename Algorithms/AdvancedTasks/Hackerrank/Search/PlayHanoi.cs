...
{
    [DisplayInfo("Hackerrank - Search", "Play Hanoi 4 Rods", typeof(List<long>))]
    class PlayHanoi
    {
        ...
        private void hanoi(int n, Peg source_peg, Peg goal_peg, Peg dev_peg, Peg axiul_peg, Dictionary<int, int> dict, ref int count)
        {
            ...
            if (n != 0)
            {
                if (source_peg.Rod.Contains(n))
                {
                    
                }
                if (goal_peg.Rod.Contains(n))
                {

                }
                if (dev_peg.Rod.Contains(n))
                {

                }
                if (axiul_peg.Rod.Contains(n))
                {
